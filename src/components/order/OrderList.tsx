...
import { getOrderStage, useCustomerOrders } from "@/hooks/use-customer-orders";
import type { OrderStage, SaleOrder, SaleOrderItem } from "@/services/orderService";

export type OrderFilter = "all" | OrderStage;

export { getOrderStage };
...
/**
 * Nama produk dari ERP memakai penanda "(BUNDLE)" untuk keperluan internal.
 * Penanda itu tidak perlu dilihat customer, jadi dibuang di sisi website.
 */
const formatProductName = (value: string) =>
  value.replace(/\s*\(\s*bundle\s*\)\s*/gi, " ").replace(/\s{2,}/g, " ").trim();

/**
 * Harga per unit dan total setelah diskon, plus penanda apakah diskonnya ada,
 * supaya harga normal bisa dicoret seperti di keranjang.
 */
const readItemPricing = (item: SaleOrderItem) => {
  const subtotal = item.subtotal;
  const totalAfterDiscount = item.total_after_discount ?? subtotal;
  const hasDiscount = totalAfterDiscount < subtotal;
  const discountedPrice =
    hasDiscount && item.quantity > 0
      ? Math.round(totalAfterDiscount / item.quantity)
      : item.price;

  return { subtotal, totalAfterDiscount, hasDiscount, discountedPrice };
};
    ...
    <article className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm md:hidden">
      ...
      <div className="divide-y divide-gray-100">
        {order.items.map((item) => {
          const { subtotal, totalAfterDiscount, hasDiscount, discountedPrice } =
            readItemPricing(item);
          const productName = formatProductName(item.product_name);

          return (
            <div key={item.id} className="flex items-start gap-3 px-4 py-3.5">
              <OrderItemImage src={item.image_url} alt={productName} />
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold leading-snug text-gray-900">{productName}</p>
                ...
                <p className="mt-1 flex flex-wrap items-center gap-1.5 text-xs">
                  ...
                  <span className={hasDiscount ? "font-semibold text-green-600" : "text-gray-500"}>
              ...
              <div className="shrink-0 text-right">
                ...
                <p
                  className={`text-sm font-semibold ${hasDiscount ? "text-green-600" : "text-gray-800"}`}
                >
                  ...
                  <td className="px-6 py-4 text-right font-semibold text-gray-900">
                    {hasDiscount ? (
                      <div className="flex flex-col items-end">
                        ...
                        <span className="text-green-600">{formatCurrency(totalAfterDiscount)}</span>
                      </div>
                    ) : (
                      formatCurrency(totalAfterDiscount)
                    )}
...
export default function OrderList({
  ...
  const { orders, isLoading, error, isLoggedIn, retry } = useCustomerOrders();
  const [page, setPage] = useState(1);
  const [pagedFilter, setPagedFilter] = useState(filter);

  // Pindah tab selalu kembali ke halaman pertama. Disetel saat render (pola
  // "adjusting state when a prop changes") supaya tidak ada render tambahan
  // yang sempat menampilkan halaman ke-3 dari tab sebelumnya.
